...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    def seqOp: (Int, WikipediaArticle) => Int = (accu: Int, w: WikipediaArticle) => if (w.mentionsLanguage(lang)) accu + 1 else accu

    def combOp: (Int, Int) => Int = (accu1: Int, accu2: Int) => accu1 + accu2
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    langs.map(lang => (lang, occurrencesOfLang(lang, rdd))).filter(_._2 > 0).sortWith((l1, l2) => l1._2 > l2._2)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] =
    index.mapValues(_.size).filter(_._2 > 0).sortBy(_._2, ascending = false).collect().toList
